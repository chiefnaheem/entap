...
export class WalletListener {
  ...
  async handleTransactionCreatedEvent(event: any): Promise<void> {
    try {
      ...
      await this.processRegularTransaction(event, senderWalletDetails, receiverWalletDetails);
    } catch (error) {
  ...
  private async handleLargeAmountTransaction(event: any, senderWalletDetails: Wallet): Promise<void> {
  ...
  private async processRegularTransaction(event: any, senderWalletDetails: Wallet, receiverWalletDetails: Wallet): Promise<void> {
    const reference = generateRandomAlphanumeric(10);
  
    await this.updateWalletsAndTransactions(event, senderWalletDetails, receiverWalletDetails, reference);
  
    this.logger.debug(`Transaction updated ${JSON.stringify(event)}`);
  }
  
  private async updateWalletsAndTransactions(event: any, senderWalletDetails: Wallet, receiverWalletDetails: Wallet, reference: string): Promise<void> {
    await this.walletService.updateWallet(event.senderWallet, {
      balance: senderWalletDetails.balance - event.amount,
      isLocked: false,
    });
